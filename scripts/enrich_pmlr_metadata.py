#!/usr/bin/env python3
"""Enrich PMLR normalized/raw records from official paper detail pages."""
...
def fetch_metadata(record: dict[str, Any], timeout: int) -> tuple[str, dict[str, Any]]:
    ...
    abstract_match = re.search(r'<div id="abstract"[^>]*>\s*(?P<abstract>.*?)</div>', text, re.S)
    parser = MetaParser()
    parser.feed(text)
    meta = parser.meta
    metadata = {
...
def main() -> None:
    ...
    raw_path = ROOT / "data" / "raw" / "pmlr" / args.conference / f"{args.conference}{args.year}.json"
